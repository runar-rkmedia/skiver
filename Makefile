...
watch:
	...
	fd -e go  | entr -r  sh -c "echo restarting...; go1.18beta1 run main.go"
gen:
	go1.18beta1 generate ./...
build-api:
	go1.18beta1 build -ldflags="${ldflags}" -o dist/skiver${SUFFIX} main.go
clean:
	rm -rf dist
	rm -rf frontend/dist
test:
...
lint:
	golangci-lint run
test-watch:
	fd -e go | entr -r sh -c 'printf "%*s\n" "${COLUMNS:-$(tput cols)}" "" | tr " " - && go1.18beta1 test ./... | grep -v "no test files"'
swagger-clean:
...
swagger-watch:
	...
	printf "base-swagger.yml\n$(fd '' types)" | entr -r go1.18beta1 generate ./...
build-web:
